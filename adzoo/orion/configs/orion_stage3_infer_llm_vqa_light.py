"""
ORION Stage3 推理配置 - 轻量版（带LLM和VQA）
基于 orion_stage3_infer.py，优化资源占用以避免死机
适用于有限资源环境（16GB显存 + 32GB内存）
"""
...
use_memory = True
num_gpus = 1  # 单GPU推理
batch_size = 1  # 最小batch_size，避免显存溢出
...
model = dict(
    type='Orion',
    save_path='./results_infer_llm_vqa/',  # 推理结果保存路径
    use_grid_mask=True,
    frozen=False,
    use_lora=True,
    fp16_infer=True,  # 🔥 关键：启用FP16推理（减少一半内存）
    # 🔥 启用LLM
    tokenizer=llm_path,
    lm_head=llm_path,
...
data = dict(
    ...
    workers_per_gpu=0,  # 🔥 单进程加载（避免LLM多进程复制）
    test=dict(
        type=dataset_type,
        data_root=data_root,
...
print("=" * 80)
print("⚠️  ORION推理配置已加载（带LLM和VQA，轻量版）")
...
print("  - 单GPU推理")
print("  - Batch Size = 1")
print("  - FP16推理: 已启用（LLM使用FP16，节省内存）")
print("  - 梯度检查点: 已启用")
print("  - 混合精度: 已启用")
print("  - Worker数量: 0（单进程，避免LLM多副本）")
print("⚠️  预计资源占用: 12-14GB显存 + 18-22GB内存（FP16优化后）")
print("=" * 80)
