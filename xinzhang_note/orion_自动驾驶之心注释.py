...
class Orion(MVXTwoStageDetector):
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        # (Pdb) img_metas[0].keys()
        # dict_keys(['filename', 'ori_shape', 'img_shape', 'lidar2img', 'pad_shape', 'gt_bboxes_3d', 'gt_labels_3d', 'scale_factor', 'flip', 'pcd_horizontal_flip', 'pcd_vertical_flip', 'box_mode_3d', 'box_type_3d', 'img_norm_cfg', 'pcd_scale_factor', 'scene_token', 'can_bus', 'folder', 'frame_idx', 'vlm_labels', 'lidar2ego'])
        # (Pdb) data.keys()
        # dict_keys(['gt_bboxes_3d', 'gt_labels_3d', 'img', 'ego_his_trajs', 'input_ids', 'gt_attr_labels', 'ego_fut_trajs', 'ego_fut_masks', 'ego_fut_cmd', 'ego_lcf_feat', 'vlm_labels', 'can_bus', 'fut_valid_flag', 'lidar2img', 'cam_intrinsic', 'timestamp', 'ego_pose', 'ego_pose_inv', 'command', 'img_feats'])
        # data:
        # (Pdb) data['gt_bboxes_3d']
        # [LiDARInstance3DBoxes(
        #     tensor([[ 2.7502e+00,  4.8448e+01, -1.8379e+00,  2.1500e+00,  5.5658e+00,
        #         2.0451e+00,  3.1416e+00,  5.1068e-12, -4.7572e-06],
        #         [ 2.5637e+00,  2.0879e+01, -1.7281e+00,  1.9668e+00,  4.0974e+00,
        #         1.5904e+00,  3.1416e+00, -0.0000e+00,  0.0000e+00],
        #         [-3.5971e+00, -4.2620e+01,  3.6373e+00,  2.7133e-01,  3.3578e-01,
        #         1.0482e+00,  1.5708e+00, -0.0000e+00,  0.0000e+00],
        #         [-6.3610e+00, -3.6867e+01,  3.5382e+00,  4.0937e-01,  1.3688e-01,
        #         1.0482e+00,  2.7846e+00, -0.0000e+00,  0.0000e+00],
        #         [ 8.2860e-01, -2.4803e+01,  3.3482e+00,  2.7187e-01,  3.3531e-01,
        #         1.0482e+00, -1.5691e+00,  0.0000e+00, -0.0000e+00],
        #         [ 3.6100e+00, -3.0549e+01,  3.4486e+00,  4.0937e-01,  1.3688e-01,
        #         1.0482e+00, -3.5695e-01,  0.0000e+00, -0.0000e+00],
        #         [ 1.5423e+01, -3.1877e+01, -7.2985e-01,  4.8921e-02,  8.4328e-01,
        #         1.6225e+00, -3.5842e-01,  0.0000e+00, -0.0000e+00],
        #         [-8.4283e+00, -4.8281e+01, -3.1407e-01,  4.8921e-02,  8.4328e-01,
        #         1.6225e+00,  1.5614e+00, -0.0000e+00, -0.0000e+00],
        #         [-1.7908e+01, -3.5184e+01,  8.0949e-02,  4.8921e-02,  8.4328e-01,
        #         1.6225e+00,  2.7832e+00, -0.0000e+00,  0.0000e+00],
        #         [ 5.6819e+00, -1.9013e+01, -3.1396e-01,  4.8921e-02,  8.4328e-01,
        #         1.6225e+00, -1.5801e+00,  0.0000e+00,  0.0000e+00]]))]
        # gt_labels_3d torch.Size([1, 10])
        # img torch.Size([1, 6, 3, 640, 640])
        # ego_his_trajs torch.Size([1, 1, 2, 2])
        # gt_attr_labels torch.Size([1, 10, 34])
        # ego_fut_trajs torch.Size([1, 1, 6, 2])
        # ego_fut_masks torch.Size([1, 1, 1, 6])
        # ego_fut_cmd torch.Size([1, 1, 1, 6])
        # ego_lcf_feat torch.Size([1, 1, 1, 9])
        # can_bus torch.Size([1, 18])
        # fut_valid_flag torch.Size([1])
        # lidar2img torch.Size([1, 6, 4, 4])
        # cam_intrinsic torch.Size([1, 6, 4, 4])
        # timestamp torch.Size([1])
        # ego_pose torch.Size([1, 4, 4])
        # ego_pose_inv torch.Size([1, 4, 4])
        # command torch.Size([1])
        # img_feats torch.Size([1, 6, 1024, 40, 40])
        # (Pdb) data['input_ids']
        # [[tensor([    1,   319, 13563,  1546,   263, 12758,  1404,   322,   385, 23116,
        #         21082, 20255, 29889,   450, 20255,  4076,  8444, 29892, 13173, 29892,
        #         322,  1248,   568,  6089,   304,   278,  1404, 29915, 29879,  5155,
        #         29889,  3148,  1001, 29901, 29871,  -200, 29871,    13,  3492,   526,
        #         19500,   263,  1559, 29889, 12148,  3867,   278, 18987, 23324,   706,
        #         363,   278,   321,  1484,  1559,  1728,  9590, 29889,   319,  1799,
        #         9047, 13566, 29901,  2266,   338,   278, 18987, 23324,   706, 32000,
        #             2], device='cuda:0')]]
        # (Pdb) data['vlm_labels']
        # [['Please provide the planning trajectory for the ego car without reasons.']]

        location = self.prepare_location(img_metas, **data) # torch.Size([6, 40, 40, 2]), 6个相机，每个相机的embed为40x40
        outs_roi = self.forward_roi_head(location, **data) # {'topk_indexes': None}
        pos_embed = self.position_embeding(data, location, img_metas) # torch.Size([1, 9600, 256])
        bbox_results = []
        if self.with_pts_bbox:
            outs, det_query = self.pts_bbox_head(img_metas, pos_embed, **data) # petr, stream petr
            # outs: dict_keys(['all_cls_scores', 'all_bbox_preds', 'dn_mask_dict', 'all_traj_preds', 'all_traj_cls_scores', 'all_traffic_states'])
            # det_query: torch.Size([1, 273, 4096])
            vision_embeded_obj = det_query.clone()
            if self.use_col_loss:
        ...
        if self.with_map_head:
            ...
            # (Pdb) outs.keys()
            # dict_keys(['all_lane_cls_one2one', 'all_lane_preds_one2one', 'all_lane_cls_one2many', 'all_lane_preds_one2many', 'outs_dec_one2one', 'outs_dec_one2many'])
            # (Pdb) map_query.shape
        ...
        if self.with_lm_head:
            ...
            vision_embeded = torch.cat([vision_embeded_obj, vision_embeded_map], dim=1) # torch.Size([1, 529, 4096])
            for i, input_ids in enumerate(data['input_ids'][0]):
                ...
                if self.use_gen_token and special_token_inputs: # must be the final round conversation
                    ...
                    ego_feature = self.lm_head.inference_ego( # llm
                    ...
                    if not self.use_diff_decoder and not self.use_mlp_decoder: # VAE-based generate 
                        ...
                        if self.PROBABILISTIC:
                            ...
                            ) # sample: torch.Size([1, 32, 1]), output_distribution: dict_keys(['present_mu', 'present_log_sigma', 'future_mu', 'future_log_sigma'])
                            distribution_comp = {**distribution_comp, **output_distribution}

                        # 2. predict future state from distribution
                        hidden_states = ego_feature.unsqueeze(1) # torch.Size([1, 1, 4096])
                        states_hs, future_states_hs = \
                            self.future_states_predict(B, sample, hidden_states, current_states)
                        # (Pdb) states_hs.shape
                        # torch.Size([6, 1, 1, 8192])
                        # (Pdb) future_states_hs.shape
                        # torch.Size([6, 1, 1, 4096])

                        ego_query_hs = \
                            states_hs[:, :, 0, :].unsqueeze(1).permute(0, 2, 1, 3) # torch.Size([6, 1, 1, 8192])
                        ego_fut_trajs_list = []
                        for i in range(self.fut_ts):
                            outputs_ego_trajs = self.ego_fut_decoder(ego_query_hs[i]).reshape(B, self.ego_fut_mode, 2)
                            ego_fut_trajs_list.append(outputs_ego_trajs)

                        ego_fut_preds = torch.stack(ego_fut_trajs_list, dim=2) # torch.Size([1, 6, 6, 2])
                    elif self.use_diff_decoder:
                        step_num = 2
