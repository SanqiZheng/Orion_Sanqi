...
class OrionHead(AnchorFreeHead):
    ...
    def temporal_alignment(self, query_pos, tgt, reference_points):
        ...
        if self.num_propagated > 0:
            # self.num_propagated 这么多个obj传递历史信息，参考stream petr
    ...
    def forward(self, img_metas, pos_embed, **data):
        ...
        x = data['img_feats'] # torch.Size([1, 6, 1024, 40, 40]), cnn 提取的每个相机的视觉特征
        B, N, C, H, W = x.shape
        num_tokens = N * H * W
        memory = x.permute(0, 1, 3, 4, 2).reshape(B, num_tokens, C) # torch.Size([1, 9600, 1024])

        memory = self.input_projection(memory) # torch.Size([1, 9600, 256])

        reference_points = self.reference_points.weight # torch.Size([600, 3])，参考点，初始化query，参考petr，stream petr
        reference_points = torch.cat([torch.zeros_like(reference_points[:self.num_extra]), reference_points], dim=0) # torch.Size([856, 3])
        reference_points, attn_mask, mask_dict = self.prepare_for_dn(B, reference_points, img_metas) # mask、padding等准备工作
        # (Pdb) reference_points.shape
        # torch.Size([1, 856, 3])
        # (Pdb) attn_mask.shape
        # torch.Size([1156, 1456])
        # (Pdb) mask_dict
        query_pos = self.query_pos(nerf_positional_encoding(reference_points.repeat(1, 1, self.n_control))) # torch.Size([1, 856, 256])
        # query_pos: linear+relu+linear
        tgt = torch.zeros_like(query_pos) # torch.Size([1, 856, 256])
        ...
        # (Pdb) tgt.shape, query_pos.shape, reference_points.shape, temp_memory.shape, temp_pos.shape, rec_ego_pose.shape
        # (torch.Size([1, 1156, 256]), torch.Size([1, 1156, 256]), torch.Size([1, 1156, 3]), torch.Size([1, 300, 256]), torch.Size([1, 300, 256]), torch.Size([1, 1456, 4, 4]))
        
        if self.use_memory :    
            current_query = self.memory_query.weight.unsqueeze(0).repeat(B,1,1) # torch.Size([1, 16, 256])
            temp_scene_query = self.memory_scene_query # torch.Size([1, 256, 256])
            # time embeding
            time_embeding = self.scene_time_embedding(pos2posemb1d(self.scene_memory_timestamp).float()) # torch.Size([1, 256, 256])
            cur_time_embeding = torch.zeros_like(current_query) # torch.Size([1, 16, 256])
            # 提取历史query的信息
            # scene query <-> memory scene query
            # mmcv\models\utils\petr_transformers.py, cross attn
        ...
        # mmcv\models\utils\petr_transformers.py, PETRTransformerDecoder, 6 layer decoder
        outs_dec = self.transformer(tgt, memory, query_pos, pos_embed, attn_mask, temp_memory, temp_pos) # torch.Size([6, 1, 1156, 256])
        if mask_dict and mask_dict['pad_size'] > 0:
            reference_points = torch.cat([reference_points[:, :mask_dict['pad_size'], :], reference_points[:, mask_dict['pad_size']+self.num_extra:, :]], dim=-2)
        else:
            reference_points = reference_points[:, self.num_extra:, :] # torch.Size([1, 900, 3])
        ...
        if mask_dict and mask_dict['pad_size'] > 0: # mask_dict['pad_size']: 100, self.num_extra: 256
            ...
            vlm_memory = outs_dec[-1, :, :self.num_extra, :] # torch.Size([1, 256, 256])
            outs_dec = outs_dec[:, :, self.num_extra:, :] # torch.Size([6, 1, 900, 256])
        if self.use_memory :
            # 提取当前query的信息
        ...
        # 6个level对应了mmcv\models\utils\petr_transformers.py的6个decoder层
        for lvl in range(outs_dec.shape[0]):
            reference = inverse_sigmoid(reference_points.clone()) # torch.Size([1, 900, 3])
            assert reference.shape[-1] == 3
            outputs_class = self.cls_branches[lvl](outs_dec[lvl]) # linear+norm+relu, torch.Size([1, 900, 9])
            if self.pred_traffic_light_state:
                outputs_traffic_state = self.tl_branches[lvl](outs_dec[lvl]) # linear+norm+relu, torch.Size([1, 900, 4])
                outputs_traffic_states.append(outputs_traffic_state)
            tmp = self.reg_branches[lvl](outs_dec[lvl]) # linear+norm+relu, torch.Size([1, 900, 10])
        ...
        all_cls_scores = torch.stack(outputs_classes) # torch.Size([6, 1, 900, 9])
        ...
        rec_can_bus = data['can_bus'].clone() # torch.Size([1, 18])
        rec_can_bus[:, :3] = 0
        rec_can_bus[:, -1] = 0
        rec_can_bus = torch.cat([data['command'].unsqueeze(-1), rec_can_bus], dim=-1) # (1, 1+18)
        memory_ego_pose = self.memory_egopose.reshape(B, -1, self.topk_proposals, 4, 4).flatten(-2) # torch.Size([1, 2, 300, 16])
        if self.output_dims is not None:
            if self.use_memory:
                # 当前query信息融入memory
                vlm_memory = torch.cat((vlm_memory, history_query), dim=1) # torch.Size([1, 272, 256]), 272=256+16
            vlm_memory = self.output_projection(vlm_memory) # torch.Size([1, 272, 4096])
            ...
            if self.with_ego_pose: 
                vlm_memory = torch.cat([vlm_memory, can_bus_embed.unsqueeze(-2)], dim=-2) # (1, 273, 4096)
            else:
        ...
        if self.use_col_loss:
            ...
            # motion_query
            batch_size, num_agent = outs_dec[-1].shape[:2] # 1, 900
            motion_query = outs_dec[-1].permute(1, 0, 2)  # torch.Size([900, 1, 256])
            mode_query = self.motion_mode_query.weight  # torch.Size([6, 256])
            # [M, B, D], M=A*fut_mode
            motion_query = (motion_query[:, None, :, :] + mode_query[None, :, None, :]).flatten(0, 1) # torch.Size([5400, 1, 256]), 两两组合，6x900
            if self.use_pe:
            ...
            motion_query = motion_query.permute(1, 0, 2) # batch first, torch.Size([1, 5400, 256])
            ...
            motion_hs = torch.cat([motion_hs, ca_motion_query], dim=-1)  # torch.Size([1, 900, 6, 512])
        

            outputs_traj = self.traj_branches[0](motion_hs) # linear+relu, torch.Size([1, 900, 6, 12])
            outputs_trajs.append(outputs_traj)
            outputs_traj_class = self.traj_cls_branches[0](motion_hs) # torch.Size([1, 900, 6, 1])
            outputs_trajs_classes.append(outputs_traj_class.squeeze(-1))
            (batch, num_agent) = motion_hs.shape[:2]
            outputs_trajs = torch.stack(outputs_trajs) # torch.Size([1, 1, 900, 6, 12])
            outputs_trajs_classes = torch.stack(outputs_trajs_classes) # torch.Size([1, 1, 900, 6])

            
        if self.use_memory:
            # update the memory bank
            out_memory = self.post_update_memory(img_metas, data, rec_ego_pose, all_cls_scores, all_bbox_preds, outs_dec, mask_dict, rec_can_bus.unsqueeze(-2), history_query) # torch.Size([1, 900, 256])
        else:
